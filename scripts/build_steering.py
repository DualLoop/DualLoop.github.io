...
def build(paper):
    ...
    for y,scale in enumerate(scales):
      for x,shape in enumerate(shapes):
        ...
        panels.append(f'<section id="category-{key}" class="category-panel" {"hidden" if x or y else ""}><h3>{label}</h3><div class="steering-samples">'+''.join(cards[:2])+'</div><a class="text-link" href="steering-gallery.html#'+key+'">More questions in this category →</a></section>')
    template=(ROOT/'content/steering-page.html').read_text().replace('{{cells}}',''.join(cells)).replace('{{panels}}',''.join(panels))
    index=(ROOT/'index.html').read_text();head=index[:index.index('<body')];head=head.replace('<title>DualLoop: Visual Question Generation through Forward–Inverse Agreement</title>','<title>Image-support steering — DualLoop</title>')
    import re
