...
def build(paper):
    ...
    index=(ROOT/'index.html').read_text();head=index[:index.index('<body')];head=head.replace('<title>DualLoop — Executable Visual Questions</title>','<title>Image-support steering — DualLoop</title>')
    import re
