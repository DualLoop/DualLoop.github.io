...
def build_story_pages(root, detailed):
    ...
    nav='''<nav class="site-nav" aria-label="Main navigation"><a class="wordmark" href="index.html">DualLoop</a><div class="nav-links"><a href="index.html#method">Overview</a><a href="questions.html">Questions</a><a href="programs.html">Programs</a><a href="steering.html">Steering</a><a href="results.html">Detailed results</a></div></nav>'''
    def write(name,title,lead,body,extra=''):
        h=re.sub(r'<title>.*?</title>',f'<title>{title} · DualLoop</title>',head,flags=re.S)
        body=re.sub(r'href="#world=', 'href="questions.html#world=',body)
        body=body.replace('href="#method"','href="index.html#method"')
        body=re.sub(r'^[ \t]+$', '', body, flags=re.M)
        (root/name).write_text(h+extra+'</head><body class="reading-page"><a class="skip-link" href="#main">Skip to content</a>'+nav+f'<main id="main"><header class="reading-page-intro"><div class="container site-width"><a href="index.html">← Project overview</a><h1>{title}</h1><p>{lead}</p></div></header>'+body+'</main><footer class="reading-footer"><a href="index.html">Project overview</a> · <a href="THIRD_PARTY_NOTICES.md">Third-party notices</a></footer></body></html>')
