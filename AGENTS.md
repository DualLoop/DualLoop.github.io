...
attribution and third-party notices. Analytics are disabled during review. Do not add tracking scripts or restore
Google Analytics without a new explicit request from the author.
`scripts/sync_navigation.py` removes legacy analytics blocks from generated pages.
